...
def render(ui: "UIController", response : "GameResponse") -> List[str]:
    ...
    stats_description = response.payload.get("stats_description", {"Health": "Determines how much damage you can take.", "Strength": "Affects the damage you deal to enemies.", "Speed": "Influences your chances to dodge", "Luck": "Can affect various random outcomes in the game."}) if step == 1 and isinstance(response.payload.get("stats_description"), dict) else {"Health": "", "Strength": "", "Speed": "", "Luck": ""}
    lines = []
    ...
    lines.extend(tk.empty_line(count=1, width=width, border_char=b_char))
    lines.extend(tk.empty_line(count=1, width=width, border_char=b_char))
    lines.extend(tk.text_block(f"Character Name: {character_name if character_name is not None else ''}", width, indent=padding, border_char=b_char))
    if step == 2 or character_name is not None:
        ...
        for stat, value in character_stats.items():
            if stat in stats_description:
                left_column.extend(tk.text_block(f"{stat}: {value}", width, indent=padding*2, border_char=""))
                right_column.extend(tk.text_block(f"{stats_description[stat]}", width, indent=padding*2, border_char=""))
        lines.extend(tk.two_column(left_column, right_column, width, border_char=b_char, col_width=(30, 70), separator="#", indent=2))
    lines.extend(tk.empty_line(count=1, width=width, border_char=b_char))
